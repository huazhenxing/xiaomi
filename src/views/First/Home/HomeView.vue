<template>
  <div class="home">
    <div class="header-bar">
      ...
      <!-- 点击切换显示不同内容 -->
      <van-tabs v-model="active" @click="onClick" title-active-color="rgb(237, 91, 0)" color="rgb(237, 91, 0)"
        background="#f2f2f2" class="top-tab" animated>
        <van-tab v-for="item in navData" :key="item.page_id" :title="item.name">
          ...
          <div v-show="item.page_id == 0">
            <div v-for="(item, index) in recommendData" :key="index">
              <!-- 轮播图 -->
              <div v-if="!item.body.h && item.body.items && !item.body.bg_color">
              ...
              <div v-if="item.body.line_color == '#f5f5f5'">
                <div style="height: 8.33px; background-color: rgb(245, 245, 245)"></div>
              </div>
              <!-- 三张图片(一大二小) -->
              <div v-if="item.body.h == 508">
                <div class="multi_cell">
                  <div class="exposure items" v-for="(item, index) in item.body.items" :key="index" :style="{
...
">
                    <img :src="item.img_url" alt="" :style="{
              ...
              <div v-if="item.body.h == 440">
                <div v-for="(item, index) in item.body.items" :key="index">
                  <img :src="item.img_url" alt="" @click="
                    clickJump(
                      item.action.type,
                      `/commodity/detail/${item.action.path}`,
                      item.action.path,
              ...
              <div v-if="item.body.line_color == '#ffffff'">
                <div style="height: 3.47rem; background-color: rgb(255, 255, 255)"></div>
              </div>
              <!-- 二张图片(小) -->
              <div v-if="!item.body.h && item.body.bg_color == '#ffffff'" class="box">
                <div class="box-item" v-for="(item, index) in item.body.items" :key="index" @click="
                  clickJump(
                    item.action.type,
                    `/commodity/detail/${item.action.path}`,
                    item.action.path,
                ...
                ">
                  <div class="img">
                    ...
                    <div class="tag2" v-for="(item, index) in item.product_tag_array" :key="index">
              ...
              ">
                <div class="ti" v-for="(item, index) in item.body.items" :key="index" @click="
                  clickJump(
                    item.action.type,
                    `/commodity/detail/${item.action.path}`,
                    item.action.path,
              ...
              <div v-if="item.body.h == 480">
                <div class="img">
                  <img v-for="(item, index) in item.body.items" :key="index" :src="item.img_url" alt="" @click="
                    clickJump(
                      item.action.type,
                      `/commodity/detail/${item.action.path}`,
                      item.action.path,
              ...
              <div v-if="item.body.h == 280">
                <div v-for="(item, index) in item.body.items" :key="index" @click="
                  clickJump(
                    item.action.type,
                    `/commodity/detail/${item.action.path}`,
                    item.action.path,
          ...
          <div v-show="item.page_id == 10288">
            <div v-for="(item, index) in intelligenceData" :key="index">
              ...
              <div v-if="item.body.items && item.body.items.length == 3">
                <HomePublicComponent :items="item.body.items" style="background-color: rgb(240, 241, 243)" />
              </div>
              <!-- 分隔线 -->
              <div v-if="
              ...
              ">
                <div style="height: 8.33px; background-color: rgb(250, 250, 250)"></div>
              </div>
              <!-- 分隔线 -->
              <div v-if="
                item.body.line_height == '24' &&
              ...
              ">
                <div style="height: 8.33px; background-color: rgb(240, 241, 243)"></div>
              </div>
              <!-- 分隔线 -->
              <div v-if="
                item.body.line_height == '24' &&
          ...
          <div v-show="item.page_id == 19071">
            <div v-for="(item, index) in televisionData" :key="index">
              ...
              <div v-if="item.body.line_height == '24'">
                <div style="height: 8.33px; background-color: rgb(255, 174, 153)"></div>
              </div>
              <!-- 商品容器(三个商品) -->
              <div v-if="item.body.items && item.body.items.length == 3">
                <HomePublicComponent :items="item.body.items" style="background-color: rgb(255, 174, 153)" />
              </div>
              <!-- 商品容器(二个商品) -->
              <div v-if="item.body.items && item.body.items.length == 2">
                <div class="television" style="background-color: rgb(255, 174, 153)">
                  <div class="box-item" v-for="(item, index) in item.body.items" :key="index" @click="
                    clickJump(
                      item.action.type,
                      `/commodity/detail/${item.action.path}`,
          ...
          <div v-show="item.page_id == 18642">
            <div v-for="(item, index) in jiadianData" :key="index">
              ...
              <div v-if="item.body.line_height === '4'">
                <div style="height: 1.38px; background-color: rgb(35, 147, 78)"></div>
              </div>
              <!-- 标题 -->
              ...
              <!-- 分隔线 -->
              <div v-if="item.body.line_height === '24'">
                <div style="height: 8.33px; background-color: rgb(225, 65, 57)"></div>
              </div>
              <!-- 商品容器(三个商品) -->
              <div v-if="item.body.items && item.body.items.length === 3">
                <HomePublicComponent :items="item.body.items" style="background-color: rgb(225, 65, 57)" />
              </div>
              <!-- 标题 -->
              ...
              <div v-if="item.body.line_height === '184'">
                <div style="height: 63.88px; background-color: rgb(225, 65, 57)"></div>
              </div>
              <!-- 标题 -->
          ...
          <div v-show="item.page_id == 13176">
            <div v-for="(item, index) in notebookData" :key="index">
              ...
              ">
                <div style="height: 13.88px; background-color: rgb(72, 120, 166)"></div>
              </div>
              <!-- 标题 -->
              <div v-if="item.body.h === 80">
                <div v-for="(item, index) in item.body.items" :key="index">
              ...
              <div v-if="item.body.btn_color === '#ea625b' && !item.body.h">
                <div class="notebook" v-for="(item, index) in item.body.items" :key="index"
                  style="background-color: rgb(72, 120, 166)" @click="
                    clickJump(
                      item.action.type,
                      `/commodity/detail/${item.action.path}`,
                      item.action.path,
              ...
              <div v-if="item.body.h === 156">
                <div class="imgs">
                  <img :src="item.img_url" alt="" v-for="(item, index) in item.body.items" :key="index" @click="
                    clickJump(
                      item.action.type,
                      `/commodity/detail/${item.action.path}`,
                      item.action.path,
              ...
              <div v-if="
                ...
                item.body.line_height === '24'
              ">
                <div style="height: 8.328px; background-color: rgb(255, 255, 255)"></div>
              </div>
              <!-- 分隔线 -->
              <div v-if="
                item.body.line_color === '#4878a6' &&
                item.body.line_height === '24'
              ">
                <div style="height: 8.328px; background-color: rgb(72, 120, 166)"></div>
              </div>
              <!-- 分隔线 -->
              <div v-if="
                item.body.line_color === '#4878a6' &&
    ...
    <div class="download-app-bottom-float" @click="getGift">
      <img src="https://cdn.cnbj1.fds.api.mi-img.com/mi-mall/831de258b72b38b8d586b387c2f54b16.png" alt="" />
    </div>
  </div>
</template>
