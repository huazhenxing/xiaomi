<template>
  ...
  <div class="card" v-if="content">
    <!-- 用户开始 -->
    <div class="user">
      <div class="user-avatar" @click="jumpPersonalView(`/micircle/personal/${content.mid}`)">
    ...
    <div class="card-content">
      ...
      <div class="article" v-if="content.title">
        ...
        <div class="images" v-if="content.pic_list.length || content.video_list.length > 0">
          <div :class="{
            image1: content.pic_list.length == 1,
            image2: content.pic_list.length == 2,
            image3: content.pic_list.length == 3,
          }" class="image" v-for="(item, index) in content.pic_list" :key="index">
      ...
      <!-- 视频开始 -->
      <div class="mi-video-player user-upload-video" v-show="content.video_list.length >= 1">
        <div class="mi-video-wrapper">
          <div class="mi-overlayer-play vertical-flex">
            <button class="mi-video-play-btn">
              <img src="https://m.mi.com/static/img/play.5f8eee7206.png" alt="" class="mi-icon mi-icon-play-lg" />
            </button>
          </div>
          <div class="mi-video-box" v-for="(item, index) in content.video_list" :key="index">
            <img :src="item.cover" />
          </div>
        </div>
      </div>
    ...
    <div class="moment-tags">
      <div class="tags-item" v-for="item in content.tag_list" :key="item.id">
        ...
        <img v-else src="https://m.mi.com/static/img/huati.d947499711.png" alt="" class="icon i-device" />
        <span>{{ item.tag }}</span>
      </div>
    </div>
    <!-- 标签结束 -->
    ...
    <div class="card-message" v-if="content.hot_comment">
      <div class="card-message-item">
        <span class="name">{{ content.hot_comment.name }}:</span>{{ content.hot_comment.comment }}
      </div>
    </div>
    <!-- 评论结束 -->
        ...
        .image {
          ...
          .player {
            ...
            &::after {
              ...
              width: 31.25rem;
              height: 31.25rem;
              background: url(https://m.mi.com/static/img/play.d0cae3f313.png) no-repeat;
