<template>
  <div class="app-view">
    <div class="hd">
      <div class="user ui-flex align-center" v-if="token > 0">
        <div class="img">
          <img src="https://s1.mi.com/m/images/m/default.png
" alt="" />
        </div>
        <div class="name">
          <p>华振兴</p>
          <p class="account">{{ token[0] }}</p>
        </div>
      </div>

      <div class="user ui-flex align-center" @click="JumpLoginView(`/fe/service/login/phone`)" v-else>
        <div class="img">
          <img src="https://m.mi.com/static/img/avatar.76a75b8f17.png" alt="" />
        </div>
        <div class="name">登录/注册</div>
      </div>

    </div>



    <div class="b1 ui-flex align-center justify-space-between">
      <div class="cite">我的订单</div>
      <div class="span ui-flex align-center" @click="JumpOrderListView(`/order/list`, 0)">全部订单</div>
    </div>

    <ul class="b2 ui-flex align-center justify-space-between">

      <li class="dfk" @click="JumpOrderListView(`/order/list`, 1)">
        <div class="icon"></div>
        <span>待付款</span>
      </li>

      <li class="dah" @click="JumpOrderListView(`/order/list`, 2)">
        <van-badge :content="all_pay_success_order_list.length > 0 ? all_pay_success_order_list.length : ''" color="#ed4d41">
          <div class="icon"></div>
          <span>待收货</span>
        </van-badge>
      </li>
      <li class="thx">
        <div class="icon"></div>
        <span>退换修</span>
      </li>
    </ul>

    <div class="ui-line"></div>

    <ul class="items">
      <li class="i-member">
        <div>
          <cite>会员中心</cite>
        </div>
      </li>
      <li class="i-wallet">
        <div>
          <cite>我的优惠</cite>
        </div>
      </li>
    </ul>
    <div class="ui-line"></div>

    <ul class="items">
      <li class="i-service">
        <div>
          <cite>服务中心</cite>
        </div>
      </li>
      <li class="i-wallet">
        <div>
          <cite>小米之家</cite>
        </div>
      </li>
    </ul>
    <div class="ui-line"></div>

    <ul class="items">
      <li class="i-fcode">
        <div>
          <cite>我的F码</cite>
        </div>
      </li>
      <li class="i-gift">
        <div>
          <cite>礼物码兑换</cite>
        </div>
      </li>
    </ul>

    <div class="ui-line"></div>

    <ul class="items">
      <li style="border-bottom: 0" @click="jumpUserSetView(`/user/set`)">
        <div>
          <cite>设置</cite>
        </div>
      </li>
    </ul>
  </div>
</template>

<script>
import Vue from 'vue';
import { Badge } from 'vant';

import { mapState, mapGetters } from "vuex";

Vue.use(Badge);
export default {
  name: "UserView",
  data() {
    return {
      token: [JSON.parse(window.localStorage.getItem('token'))] || [],
      pay_success_order_num: null
    }
  },
  created() {

  },
  methods: {
    JumpLoginView(path) {
      this.$router.push(path);
    },

    JumpOrderListView(path, active) {
      this.$router.push(path + `?active=${active}`);
    },

    jumpUserSetView(path) {
      this.$router.push(path);
    }
  },

  computed: {
    ...mapState(["all_pay_success_order_list"]),
    ...mapGetters(["order_list_goods_count"]),
  },
};
</script>

<style lang="scss" scoped>
cite {
  font-size: 14.5833rem;
  font-style: normal;
  color: rgba(0, 0, 0, .87);
}

.app-view {
  padding-bottom: 50rem;

  .hd {
    background: url(https://m.mi.com/static/img/bg.63c8e19851.png) center 0 #f37d0f;
    background-size: auto 100%;
    padding: 16.667rem 0;

    .user {
      .img {
        margin: 0 8.333rem 0 16.667rem;
        width: 44rem;
        height: 44rem;
        overflow: hidden;
        box-sizing: border-box;
        border-radius: 100%;
        border: 3px solid hsla(0, 0%, 100%, 0.4);
        text-align: center;

        img {
          width: auto;
          height: 100%;
          margin: 0 auto;
        }
      }

      .name {
        color: #fff;
        font-size: 12.5rem;
        text-align: left;

        .account {
          color: hsla(0, 0%, 100%, .6);
        }
      }
    }
  }

  .b1 {
    background-color: #fff;
    height: 40.664rem;
    border-bottom: 1px solid rgba(0, 0, 0, 0.15);
    color: rgba(0, 0, 0, 0.54);
    position: relative;
    padding-right: 29.167rem;

    .cite {
      font-size: 14.5833rem;
      padding: 0 0 0 16.6667rem;
      color: rgba(0, 0, 0, 0.87);
    }

    .span {
      font-size: 12.5rem;
      color: rgba(0, 0, 0, 0.87);
    }

    &::after {
      content: "";
      position: absolute;
      right: 16.667rem;
      top: 50%;
      width: 9.414rem;
      height: 9.414rem;
      border-left: 1px solid currentColor;
      border-top: 1px solid currentColor;
      transform: translate3d(0, -50%, 0) rotate(135deg);
      -webkit-transform: translate3d(0, -50%, 0) rotate(135deg);
    }
  }

  .b2 {
    padding: 0 41.667rem;
    background-color: #fff;

    li {
      display: flex;
      position: relative;
      flex-direction: column;
      align-items: center;
      padding: 20.833rem 0;
    }

    span {
      color: rgba(0, 0, 0, 0.87);
      font-size: 12rem;
    }

    .dfk {
      .icon {
        width: 25rem;
        height: 25rem;
        margin: 0 auto 8.333rem;
        background: url(data:image/png;base64,) no-repeat center 0;
        background-size: 100% auto;
      }
    }

    .dah {
      .icon {
        width: 25rem;
        height: 25rem;
        margin: 0 auto 8.333rem;
        background: url(data:image/png;base64,) no-repeat center 0;
        background-size: 100% auto;
      }
    }

    .thx {
      .icon {
        width: 25rem;
        height: 25rem;
        margin: 0 auto 8.333rem;
        background: url(https://m.mi.com/static/img/nav-4.d68723895f.png) no-repeat center 0;
        background-size: 100% auto;
      }
    }
  }

  .ui-line {
    height: 10.414rem;
    background: #f5f5f5;
    overflow: hidden;
    clear: both;
  }

  .items {
    background: #fff;

    li {
      position: relative;
      color: rgba(0, 0, 0, 0.54);
      background: url(https://m.mi.com/static/img/i-setting.fb9625b3f2.png) no-repeat 17rem;
      background-size: 25rem auto;
      border-bottom: 1px solid rgba(0, 0, 0, 0.15);
      line-height: 0rem;
      list-style: none;

      div {
        display: block;
        padding-right: 29.167rem;
        padding-left: 58.333rem;
        height: 54.164rem;
        display: box;
        display: -webkit-box;
        box-align: center;
        -webkit-box-align: center;
        box-pack: justify;
        -webkit-box-pack: justify;
        color: currentColor;
      }

      &::after {
        content: "";
        position: absolute;
        right: 16.667rem;
        top: 50%;
        width: 9.414rem;
        height: 9.414rem;
        border-left: 1px solid currentColor;
        border-top: 1px solid currentColor;
        transform: translate3d(0, -50%, 0) rotate(135deg);
        -webkit-transform: translate3d(0, -50%, 0) rotate(135deg);
      }

      &::before {
        content: "";
        position: absolute;
        left: 0;
        top: -2px;
        height: 3px;
        background: #fff;
        width: 58.328rem;
      }
    }

    li.i-member {
      background-image: url(data:image/png;base64,);
    }

    .i-wallet {
      border-bottom: 0;
      background-image: url(data:image/png;base64,);
    }

    .i-service {
      background-image: url(data:image/png;base64,);
    }

    .i-wallet {
      border-bottom: 0rem;
      background-image: url(data:image/png;base64,);
    }

    .i-fcode {
      background-image: url(data:image/png;base64,);
    }

    .i-gift {
      border-bottom: 0;
      background-image: url(https://m.mi.com/static/img/i-gift.ece9aa1222.png);
    }
  }
}

.ui-flex {
  display: -webkit-box;
  display: flex;
}

.align-center {
  -webkit-box-align: center;
  align-items: center;
}

.justify-space-between {
  -webkit-box-pack: justify;
  justify-content: space-between;
}

:after,
:before {
  text-decoration: inherit;
  vertical-align: inherit;
}

*,
:after,
:before {
  box-sizing: inherit;
}
</style>